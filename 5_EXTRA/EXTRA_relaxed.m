function out = EXTRA_relaxed(Settings)
% Compute the worst-case performance of t steps of EXTRA [1] under the 'Settings' provided and
...
assert(strcmp(type,'spectral_relaxed'),"EXTRA_relaxed only applies to spectral description of the averaging matrix (range of eigenvalues)");

if verbose
    fprintf("------------------------------------------------------------------------------------------\n");
    fprintf("Settings provided for the relaxed PEP:\n");
...
% Pb = [xb0 gb0...gb^t gcb]
% Pp = [xp0 gp0...gp^t Wxp0...Wxp^t-1 gcp gsp]
% F = [f0..ft fc]
...
xb(:,2) = xb(:,1) - alpha(1)*gb(:,1);
xp(:,2) = Wxp(:,1) - alpha(1)*gp(:,1);
for k = 2:t
    xb(:,k+1) = 2*xb(:,k) - xb(:,k-1) - alpha(k)*(gb(:,k) - gb(:,k-1));
    xp(:,k+1) = xp(:,k) + Wxp(:,k) -1/2*(xp(:,k-1) + Wxp(:,k-1)) - alpha(k)*(gp(:,k) - gp(:,k-1));
end
...
switch init.x
    ...
    case {'navg_it_err_combined_grad','2'}
        ...
        cons = cons + ((xb(:,1)-xsb).'*Gb*(xb(:,1)-xsb) + (xp(:,1)-xsp).'*Gp*(xp(:,1)-xsp) + init.gamma*( (gp(:,1)).'*Gp*(gp(:,1)) ) <= init.E^2 );
...
switch perf
    ...
    case {'navg_it_err_combined_grad','2'}  % (avg_i ||xi^k - xs||^2) + gamma* (avg_i ||gi^k - avg_i(gi^k)||^2)
        obj = (xb(:,t+1)-xsb).'*Gb*(xb(:,t+1)-xsb) + (xp(:,t+1)-xsp).'*Gp*(xp(:,t+1)-xsp) + init.gamma*( (gp(:,t+1)).'*Gp*(gp(:,t+1)) );
