function out = DGD_relaxed(Settings)
% Compute the worst-case performance of t steps of DGD [1] under the 'Settings' provided and
...
assert(strcmp(type,'spectral_relaxed'),"DGD_relaxed only applies to spectral description of the averaging matrix (range of eigenvalues)");
...
% DGD iterates coeff
for k = 1:t
    xb(:,k+1) = xb(:,k) - alpha(k)*gb(:,k);
    xp(:,k+1) = Wxp(:,k) - alpha(k)*gp(:,k);
end
